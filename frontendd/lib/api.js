...
export async function linkWallet({ user_id, wallet_address, message, signature }) {
  const res = await fetch(`${BACKEND_URL}/wallet/link`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ user_id, wallet_address, message, signature }),
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
...
export async function depositParams(amount) {
  const res = await fetch(`${BACKEND_URL}/bridge/deposit-params`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ amount_usdc_units: amount }),
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
...
export async function bridgeStatus(burnTxHash) {
  const res = await fetch(`${BACKEND_URL}/bridge/status/${burnTxHash}`, {
    cache: "no-store",
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
