...
This returns an `agent_address`. Tell the human to approve that exact
address from their own wallet via Hyperliquid's `approve_agent` action
(this is a signature they make themselves — you cannot do this for
them, and Alias never sees their private key). Once they confirm it's
approved, you're done with setup permanently for this user.
