...
const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || "https://agenthub-wine.vercel.app";
  ...
  callbacks: {
    async jwt({ token, account, profile }) {
      if (account?.provider && profile) {
        ...
        if (!res.ok) throw new Error(`Backend register failed: ${res.status} ${await res.text()}`);
    ...
    async session({ session, token }) {
      if (session.user) session.user = { ...session.user, id: token.userId };
      return session;
    },
  },
});
