...
import { BACKEND_URL, confirmPermissions } from "../../lib/api";
import { approveAgent } from "../../lib/hyperliquid";
import { depositUSDC } from "../../lib/deposit";

const STEPS = [
  { title: "Sign in", desc: "Continue with Google" },
...
export default function Onboarding() {
  ...
  async function setupWallet() {
    if (!walletClient || !address || !session?.user?.id || !session.user.googleIdToken) {
      return;
    }
    ...
    try {
      ...
      const googleIdToken = session.user.googleIdToken;

      const nonceResponse = await fetch(`${BACKEND_URL}/wallet/nonce`, {
        method: "POST",
        headers: {
      ...
      if (!linkResponse.ok) {
        throw new Error(await linkResponse.text());
      }
  ...
  return (
    <main className="max-w-md mx-auto px-5 py-16">
      ...
      {step === 0 ? (
        ...
        <button onClick={fundWallet} disabled={loading} className="w-full bg-signal text-[#071a2e] font-mono font-semibold py-2.5 rounded">
          {loading ? "Bridging USDC..." : "Deposit 1 USDC"}
        </button>
      )}
