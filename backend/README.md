...
4. `POST /wallet/link` with a testnet Arc address → get back an agent address
5. Approve that agent address manually via Hyperliquid's testnet UI/SDK
6. `POST /bridge/deposit` with testnet USDC → confirm funds show up on Hyperliquid testnet
7. `POST /agents/{arc_address}/trade` → confirm a position opens
8. `GET /dashboard/{arc_address}` → confirm it reflects the position
9. `POST /agents/{arc_address}/close` → confirm the position closes
